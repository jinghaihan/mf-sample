...
export function initRouter () {
  ...
  if (!window.__POWERED_BY_QIANKUN__) {
    router.beforeEach((to, from, next) => {
      const loggedIn = Vue.ss.get('loggedIn')
      if (loggedIn || !window.microApp.getCustomConfig('needToLogin')) {
  ...
  function handleBeforeEach (to, from, next) {
    const getters = store.getters['user/GET_ROUTES']
    // 判断当前是否已经处理过路由信息
...
// 路由前缀
const routesPrefix = window.__POWERED_BY_QIANKUN__ ? `/micro-${config.name}/` : '/'
...
export function generatorRouter (data) {
  return new Promise((resolve, reject) => {
    data.forEach(route => {
      const currentRoute = {
        // 路由路径，target地址
        path: routesPrefix + route.target,
        // 路由名称
        name: route.target,
        // 该组件对应的路由组件
